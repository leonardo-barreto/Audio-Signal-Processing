...
function [outputSignal,outputBlockSizes] = TimeVarying_ResampleDEBUG(inputSignal,sinAnalysisParameters,resampleFactors,filterCoeffs,reverseBlockSizes,DEBUG)
    ...
    %Resampling
        ...
        if length(reverseBlockSizes) > 0
            totalBlocks = length(reverseBlockSizes);
        else
        ...
        while (blockIndex <= totalBlocks)
            ...
            if length(reverseBlockSizes) > 0
                blockSize = reverseBlockSizes(blockIndex);        % Blocks are disjoint, i.e. don't include the sample that starts the next one.
            else
            ...
            if DEBUG == 1
                ...
                disp(X)
                X = sprintf('Step Size: %d', stepSize);
                disp(X)
            ...
            outputBlockSamples(blockIndex) = resampledOutputIndex-1;
            blockIndex = blockIndex + 1;   
        end

        outputBlockSizes = outputBlockSamples - delayseq(outputBlockSamples',1)';

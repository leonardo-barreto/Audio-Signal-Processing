...
sig_name = 'opera_fem4'; % for reading audio signal and naming the resulting figures
...
plot_spec(sprintf('%s', sig_name), compress_dB_norm(abs(spec),80), time, freq, plot_par, figs_path);
return
% Plot signal and spectrogram - - - - - -
...
%     if plot_flags(1)
%         plot_spec(sprintf('%s_%d', sig_name, N_w(ind)), spectrograms_tensor(:,:,ind).^.25, time, freq, plot_par, figs_path);
%     end
% end
% 
% % N_w_str = '';

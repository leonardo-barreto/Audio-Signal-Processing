function [spectrograms, freq, t ] = spectrogram_tensor_prep(input_sig, fs, N_w, overlap_short)
...
[spec_short, freq, t] = spectrogram(input_sig, N_w(1), overlap_short, N_w(end), fs);
spectrogramsBeforeTrim{1} = spec_short;

for ind = 2:length(N_w)
    x_shifted = [zeros((N_w(ind) - N_w(1))/2,1); input_sig];
    spectrogramsBeforeTrim{ind} = spectrogram(x_shifted, N_w(ind), N_w(ind) - N_w(1) + overlap_short, N_w(end), fs);
end
...
for ind = 1:length(N_w)
    spectrograms(:,:,ind) = (abs(spectrogramsBeforeTrim{ind}(:, 1:spec_length)));
end

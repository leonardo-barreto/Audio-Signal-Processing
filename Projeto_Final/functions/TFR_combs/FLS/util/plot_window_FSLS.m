function plot_window_FSLS(size_w, name, font_size)
% PLOT windows
...
if mod(size_w(1), 2) == 0
    size_w(1) = size_w(1) + 1;
    fprintf('WARNING: size_w(1) must be an odd number! Using %d instead!\n', size_w(1));
end

if mod(size_w(2),2) == 0
    size_w(2) = size_w(2) + 1;
    fprintf('WARNING: size_w(2) must be an odd number! Using %d instead!\n', size_w(2));
end
...
W = maskc.*maskr;
x_ticks = -(size_w(2)-1)/2 : (size_w(2)-1)/2;
y_ticks = -(size_w(1)-1)/2 : (size_w(1)-1)/2;

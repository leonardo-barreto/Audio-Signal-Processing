function [spectrograms, freq, t ] = spectrogram_tensor_prep_binwise(input_sig, fs, N_w, overlap_short, N_FFT)
...
[spec_short, freq, t] = spectrogram(input_sig, N_w(1), overlap_short, N_FFT, fs);
spectrogramsBeforeTrim{1} = spec_short;

for ind = 2:length(N_w)
    x_shifted = [zeros((N_w(ind) - N_w(1))/2,1); input_sig];
    spectrogramsBeforeTrim{ind} = spectrogram(x_shifted, N_w(ind), N_w(ind) - N_w(1) + overlap_short, N_FFT, fs);
end
